...
- **Model URL** (`resolveGltfUrl` in `src/lib/gltfAsset.ts`): order is `NEXT_PUBLIC_GLTF_URL` (full override) → optional **`NEXT_PUBLIC_MESH_RUN_ID`** plus `GET /geo-nyc-proxy/api/run/{id}` to resolve the run’s mesh → else runs from `GET /geo-nyc-proxy/api/runs` ranked by `(succeeded > others)` then `mode tier` (PDF-driven `document_llm_dsl` runs from `make seed-runs` rank above fixture runs) then `created_at` newest-first, probing **`/geo-nyc-proxy/static/exports/{run_id}/model.glb`** (or the mesh filename from the manifest) → legacy `static/exports/sample.glb` → `public/exports/sample.glb` → **Khronos Duck** so the dock always has geometry.
